...
export default function Inbox() {
  ...
  const archiveAllEmails = () => {
    const selectedIds = Object.keys(checkedItems)
      .filter((id) => checkedItems[Number(id)])
      .map(Number);

    const archivedItems = JSON.parse(localStorage.getItem("Data") || "[]");
    const updatedArchived = [
      ...archivedItems,
      ...data.filter((item) => selectedIds.includes(item.id)),
    ];
    localStorage.setItem("Data", JSON.stringify(updatedArchived));
  ...
  return (
    <div>
      <div className="flex flex-row px-5 py-1 items-center bg-gray-100 border-t border-b py-2 border-gray-200 w-full justify-between">
        ...
        {deletedisplay && (
          <div className="flex flex-row-reverse gap-2 items-center">
            <div
              ...
              className="text-red-500 cursor-pointer hover:scale-105 transition text-lg"
            >
              <RiDeleteBin6Line />
            </div>
            <div
      ...
      <div className="w-full flex flex-row">
        {display && (
          <div className="w-full h-[85vh] flex flex-col gap-4 px-4 py-4 overflow-auto border-r border-gray-300">
            {data.map((item: Item) => (
              <div
                onClick={() => {
                      ...
                      <div className="flex flex-col justify-between h-[74vh]">
                        ...
                        <div>
                          ...
                          <div className="flex border py-1 pr-1 rounded-full">
                            ...
                            />
                            <p
                              onClick={() => savesentmails(item.subject)}
